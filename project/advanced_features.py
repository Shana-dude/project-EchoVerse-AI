        ...
        if st.session_state.get('generated_summary'):
                ...
                if st.button("🎵 Generate Audio Summary"):
                    if 'generated_summary' in st.session_state:
                        with st.spinner("🎤 Generating audio summary..."):
                            ...
                            if audio_data:
                                st.session_state.summary_audio = audio_data
            ...
            with col3:
                if st.button("📧 Share Summary"):
                    if 'generated_summary' in st.session_state:
                        self._show_sharing_options(st.session_state.generated_summary)
                    else:
                        st.warning("⚠️ Please generate a summary first!")

    def _show_sharing_options(self, summary_text):
        """Show sharing options for WhatsApp and Email"""
        ...
        with col1:
            ...
            whatsapp_text = f"📋 EchoVerse Summary: {summary_text} 🎧 Generated by EchoVerse AI"
            encoded_whatsapp = whatsapp_text.replace(' ', '%20').replace('\n', '%0A')
            whatsapp_url = f"https://wa.me/?text={encoded_whatsapp}"
            ...
            if st.button("📱 Share on WhatsApp", use_container_width=True):
                ...
                st.success("✅ WhatsApp link generated! Click the link above.")

        with col2:
            st.markdown("#### 📧 Email")
            # Create email content
            email_subject = "EchoVerse AI Summary"
            email_body = f"Hi,\n\nI wanted to share this summary generated by EchoVerse AI:\n\n{summary_text}\n\nBest regards,\nGenerated by EchoVerse"
            encoded_email = email_body.replace(' ', '%20').replace('\n', '%0A')
            email_url = f"mailto:?subject={email_subject}&body={encoded_email}"

            if st.button("📧 Share via Email", use_container_width=True):
                st.markdown(f'<a href="{email_url}">Click here to open Email</a>', unsafe_allow_html=True)
                st.success("✅ Email link generated! Click the link above.")

        # Copy to clipboard option
        st.markdown("#### 📋 Copy to Clipboard")
        st.text_area("Summary Text (Copy this):", summary_text, height=150, key="copy_summary")
        st.info("💡 Select all text above and copy (Ctrl+C) to share anywhere!")
