...
from gemini_integration import GeminiTextGenerator
...
class TextProcessor:
    def __init__(self):
        ...
        self.chapters = []
        self.summary = ""
        self.gemini_generator = GeminiTextGenerator()
    ...
    def show_text_input_interface(self):
        ...
        tab1, tab2, tab3 = st.tabs(["✍️ Paste Text", "📁 Upload File", "🤖 Generate from Topic"])
        ...
        with tab3:
            self._show_topic_generation_interface()
...
The mysterious object landed softly in the meadow behind her house, glowing with an ethereal blue light. Luna's heart raced with excitement and curiosity as she made her way toward the strange visitor from the sky."""
    ...
    def _show_topic_generation_interface(self):
        """Show topic-based text generation interface"""
        st.markdown("### 🤖 Generate Text from Topic")
        st.markdown("Enter a topic and let AI generate content for you using Google Gemini.")

        # Topic input
        topic = st.text_input(
            "Enter your topic:",
            placeholder="e.g., The benefits of renewable energy, A story about space exploration, How to learn programming...",
        ...
        with col1:
            word_count = st.selectbox(
                ...
                index=1,
                help="Choose how long you want the generated content to be"
            )

        with col2:
            content_type = st.selectbox(
                "Content type:",
                ["Article", "Story", "Essay", "Tutorial", "Blog Post"],
                help="Choose the style of content to generate"
            )
        ...
        if st.button("🚀 Generate Content", type="primary", use_container_width=True):
            ...
            try:
                ...
                result = self.gemini_generator.generate_text_from_topic(
                    topic=topic,
        ...
        # API status check
        if st.button("🔍 Test Gemini API Connection", type="secondary"):
            try:
                is_working, message = self.gemini_generator.test_api_connection()
                if is_working:
